  ...
  compileNode: (o) ->
    ...
    for node, index in @expressions
      ...
      if node instanceof Block
        ...
        node.front = true
        if not o.indent
          if node instanceof Assign and node.value instanceof Code
            node.value.shouldExport = yes
          else if node instanceof Assign
            node.variable.shouldExport = yes
...
exports.Yield = class Yield extends Base
  ...
  compileNode: (o) ->
	  # TODO support the callback param
    [@makeCode("yield(#{
	    ["#{@expression.compile o, LEVEL_PAREN}" if @expression]})")]
    ...
    unless @context
      ...
      unless varBase.hasProperties?()
        if @param
          ...
          if not found
            shouldExport = if @variable.shouldExport then 'export ' else '' 
            compiledName.unshift (new CodeFragment @variable, 
              shouldExport + 'var ')
...
exports.Code = class Code extends Base
  ...
  compileNode: (o) ->
    ...
    # TODO check
    functionToken += '*' if @body.match [ Yield, -> ]
